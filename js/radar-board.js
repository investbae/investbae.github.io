    ...
    (function () {
      ...
      function safeUrl(u){ u=u||""; if(!/^https?:\/\//i.test(u) || /\/DRF\/|[?&]OC=|lawService\.do/i.test(u)) return "https://www.law.go.kr"; return u; }
      function fmtDate(d){ var m=(d||"").replace(/-/g,"").match(/(\d{4})(\d{2})(\d{2})/); return m? m[1]+"."+m[2]+"."+m[3] : (d||""); }
      ...
      function statusBadge(r){
        ...
        if(r.ef_date && /\d{4}/.test(r.ef_date)){
          return r.future ? '<span class="badge upcoming">공개자료상 시행 예정</span>'
                          : '<span class="badge arrived">공개자료상 시행일 도래</span>';
        }
        return '<span class="badge unconfirmed">절차단계 미확인</span>';
      }
      function daysUntil(d){
        var m=(d||"").replace(/-/g,"").match(/(\d{4})(\d{2})(\d{2})/); if(!m) return null;
        var t=new Date(+m[1], +m[2]-1, +m[3]); var now=new Date(); now.setHours(0,0,0,0);
        return Math.round((t-now)/86400000);
      }
      function gate(r){
        var src = r.dept ? esc(r.dept) : "법제처";
        var checked = r.collected_date || META_DATE;
        var ef = (r.ef_date && /\d{4}/.test(r.ef_date)) ? "🗓️ 시행 "+fmtDate(r.ef_date) : "🗓️ 시행일 미확인";
        return '<div class="gate"><span>🏛️ <b>출처</b> '+src+'</span><span>✍️ 수동확인</span><span>🗓️ 확인일 '+esc(checked)+'</span><span>'+ef+'</span></div>';
      }
      function card(r){
        var titleTxt = esc(r.title);
        var days = r.future && r.ef_date ? daysUntil(r.ef_date) : null;
        var daysTxt = (days!==null && days>=0) ? '<div class="days">시행예정일까지 '+days+'일</div>' : '';
      ...
      function applyFilter(f){
        ...
        if(shown===0 && !board.querySelector(".empty-f")) {
          var e=document.createElement("div"); e.className="empty empty-f"; e.textContent="현재 공개자료 대조 완료 항목 없음 (해당 분야)"; board.appendChild(e);
        } else { var ex=board.querySelector(".empty-f"); if(ex && shown>0) ex.remove(); }
      }
      fetch("/data/radar.json?cb="+Date.now()).then(function(r){return r.json();}).then(function(d){
        META_DATE = d.collected_date||"";
        document.getElementById("meta").innerHTML =
          '최종 확인 <b>'+esc(d.collected_date||"")+'</b> · 출처 <b>'+esc(d.source||"법제처 국가법령정보")+'</b> · 관측 키워드 <b>'+esc((d.keywords&&d.keywords.length)?d.keywords.join("·"):"벤처투자·벤처기업·가상자산·신기술사업금융 등")+'</b>';
        // recent/upcoming/ppc 통합
        BOARD = [].concat(d.recent||[], d.upcoming||[]);
        var board=document.getElementById("board");
        if(!BOARD.length){ board.innerHTML='<div class="empty">현재 공개자료 대조 완료 항목 없음.</div>'; }
        else { board.innerHTML = BOARD.map(card).join(""); }
        applyFilter(activeField);
        // 캘린더: 원문 시행일 확인 + 미래 항목만
        var cal = (d.upcoming||[]).concat(d.recent||[]).filter(function(r){ return r.ef_date && /\d{4}/.test(r.ef_date) && r.future; });
        var calEl=document.getElementById("cal");
        if(!cal.length){ calEl.innerHTML='<div class="empty">원문 시행일이 확인된 시행 예정 항목이 현재 없습니다. 이 칸은 관측 키워드에 해당하는 개정 법령이 공포된 뒤 시행일이 도래하기 전인 기간에만 채워지며, 이미 시행된 항목은 위 관측 보드에서 확인하실 수 있습니다.</div>'; }
        else {
